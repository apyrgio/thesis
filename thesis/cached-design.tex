...
The most important guarantee is provided by the lock in hash table. When a 
thread wants to do a xhash operation (insert, lookup, remove), it must acquire a 
specific lock. This applies to both hash tables, "entries" and "rm_entries".  
Also, there are cases where data have to migrate from one table to the other. In 
this case, the locks are acquired in an order that prevents deadlocks.
