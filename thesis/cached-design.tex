...
enqueued. The thread that enqueues a job can attempt to execute it too, by 
acquiring a lock for the workq. If the lock is unheld, the thread will acquire 
and will be able to execute the enqueued job. Else, it can safely leave and its 
job will be executed by the thread that has holds the lock.
