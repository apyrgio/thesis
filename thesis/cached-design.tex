...
The following two chapters are the main bulk of this thesis and they present 
...
xtype. The reasons why we have chosen to use a hash table are:
...
dictobject, and is a feature of other hash tables too, is its dynamic resizing 
ability once it has reached the two thirds (2/3) of its capacity. This is the 
...
created to hold only a fraction of \(n\), one (1) in the worst case. In this 
case, the number of rehashes will be \(n + n/2 + n/4 + n/8 + ... + 1 = 2n\).  
This means that the \textbf{amortized} cost of rehashes will be \(2n/n\), which 
is practically O(1) and thus negligible.  The same amortized cost applies to 
the copies of the entries on reallocation.
\end{quotation}
