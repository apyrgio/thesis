...
\begin{inparaenum}[i)]
	\item create a VM over Archipelago with support from cached and
...
VM. Once the VM was started successfully, we installed FIO
...
Our aim has been to simulate the workloads of the previous categories, in order 
to produce comparable results. The FIO job file that was created for this 
...
Moreover, the fact that the benchmark is done on a file means the following two 
things:

\begin{enumerate}
	\item That we are bound to include in our measurements the overhead of 
		filesystem operations and the I/O scheduler of the VM.  This is 
		actually a welcoming result, since it provides a more accurate 
		measurement of the I/O speed that the common user is likely to 
		experience.
	\item That we can be sure that only the first write to the file will be 
		affected by Copy-On-Write while the subsequent writes will behave 
		normally. Thus, we can discard the first write and use only the 
		subsequent ones.
\end{enumerate}
...
The bandwidth results for our evaluation can be seen in Figure \ref{bw-vm.pdf} 

\diagram{Bandwidth performance of cached}{bw-vm.pdf}

\diagram{Latency performance of cached}{lat-vm.pdf}

We will compare the results of each configuration with the results of previous 
benchmarks:
