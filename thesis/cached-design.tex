...
time. This means that xworkq is generally used when multiple threads want atomic 
...
In cached context, it is used when we have received a read/write request for an 
object and we want to atomically acess/modify the requested data range.
...
It consist of a queue where jobs (e.g.  read from block, write to block) are 
