...
In writeback mode, the data are written directly to cache...
In writethrough mode, the data are written to the backing storage and then to 
cache...

Since Archipelago divides internally the volumes to objects (usually 4MB), the 
cached peer operates on object level. Also, in order to know which parts of the 
cached object are actually written, or are in the process of being read etc.  
cached further divides objects to the next logical entity, buckets (typically 
4KB). Each bucket consists of each data and metadata and cannot be half-empty, 
or half-allocated. Thus, a bucket can be considered as the quantum of cached 
objects.

The cached peer consists of a number of building blocks. Per Archipelago policy, 
these building blocks have been written in the xtypes fashion. The reasons 
behind this decision have been discussed in chapter ?
...
\begin{itemize}
	\item xcache, an xtype that mainly provides indexing support
	\item xworkq, an xtype that guarantees atomicity for execution of jobs 
		on the same target
	\item xwaitq, an xtype that allows condition execution of jobs
	\item bucket pool, a pre-allocated memory pool for buckets and their 
		metadata
\end{itemize}
