...
the cache size or its write policy. In this category, we will further quantify 
our previous assumptions about the inner workings of cached by measuring how
cached scales with the number of threads and the overhead of the index 
mechanism.
...
Measuring the performance impact of multi-threading is meaningless without 
issuing any parallel requests. Therefore, we will test each number of cached 
threads against a range of parallel requests and measure its performance.
