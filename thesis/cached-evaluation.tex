...
usually the most common one that is benchmarked.  
...
\section{Performance comparison between cached and sosd}\label{sec:perf-plot}
...
\diagram{bw-write-threads.pdf}{Bandwidth performance per number of threads}
\diagram{lat-write-threads.pdf}{Latency performance per number of threads}
...
For this reason, we have written 128K (where \textit{K} is 1024 and \textit{M} 
is 1024\^2), 256K, 512K and 1M objects and have measured their latency 
performance. We expect that the experimental results will verify the claim that 
our implementation is O(1).
...
\diagram{cold1.pdf}{Latency performance of cold/warm cache for variable sizes}
