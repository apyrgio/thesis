...
The insertion process involves all xcache elements and has some many steps, 
...
\begin{enumerate}
	...
	\item Next, we search the active entries to ensure that no other thread 
		has already inserted that entry. If however our entry is there, 
		we return and free the entry we have previously allocated
	\item Then, we search the evicted entries to likewise ensure that our 
		entry has not
\end{enumerate}
