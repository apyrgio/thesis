...
xcache requests the following information from the peer:
...
\begin{description}
	...
	\item[flags:] The flags blabla the following two things:
		\begin{enumerate}
			\item The LRU algorithm. For the cached implementation, we use the 
				O(1) LRU, but xcache also allows to use two more LRU 
				algorithms, a binary priority heap (O(log(N))) or an LRU array 
				(O(N)).
			\item The usage of the hash table for evicted entries. Although our 
				implementation relies heavily on it, this does not account for 
				all the other peers that use xcache.
		\end{enumerate}
	\item[priv:] A pointer (void *) to a structure that will be returned when 
		...
		the xcache struct and relevant only to the top caller. We initialize it 
		with the cached struct.
\end{description}
...
Moreover, the typical fields that are definition of the \texttt{xcache entry} 
struct is shown in Listing \ref{lst:xcache-entry.h}.
...
More importantly, these are the functions which are related to indexing and 
\xcache exposes to the peer:
...
struct fields related to indexing and explain how they are used by each 
function.

\ccode{xcache entry struct relevant indexing}{xcache-entry-index.h}

The way each function handles the above xcache and xcache_entry fields follows: 

\textbf{xcache\_index}

(Re)inserts the entry to the entries hash table. Then it updates position the 
entry to the head of tail of the LRU queue (or the head of the MRU queue, 
depending on what's your standpoint) and updates the younger/older fields of 
its neighbors. Moreover, if the entry was reinserted, it makes its state 
ACTIVE.

\textbf{xcache\_lookup}
